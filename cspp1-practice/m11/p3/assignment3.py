...
and display that hand to the user. We can also ask the user for
a word (Python's input) and score the word (using your getWordScore).
However, at this point we have not written any code to verify that a
word given by a player obeys the rules of the game. A valid word is in
the word list; and it is composed entirely of letters from the current
...
is not a valid word - if you code this function correctly, you shouldn't need
...
def is_valid_word(word, hand, word_list):
    ...
    for i in word:
        if i in hand:
            count += 1
    if count == len_w and word in word_list:
        return True
    return False
...
def main():
    ...
    l_letter = input().split()
    print(is_valid_word(word, adict, l_letter))
